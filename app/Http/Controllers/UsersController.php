       ...
       'confirmPassword.required' => 'password does not match',
    	...
    	if($request->isMethod('post')){
    		...
    		if($usersCount>0){
               ...
               // $user->confirmPassword = bcrypt($data['confirmPassword']);
               ...
               // Send Register Email
                ...
                Mail::send('emails.register',$messageData,function($message) use($email){
                    $message->to($email)->subject('Registration with Brahmastra Website');
                });

                // Send Confirmation Email
                $email = $data['email1'];
                $messageData = ['email'=>$data['email1'],'name'=>$data['name'],'code'=>base64_encode($data['email1'])];
                ...
                if(Auth::attempt(['email'=>$data['email1'],'password'=>$data['password']])){
                    Session::put('frontSession',$data['email1']);

                    if(!empty(Session::get('session_id'))){
                        $session_id = Session::get('session_id');
                        DB::table('cart')->where('session_id',$session_id)->update(['user_email' => $data['email1']]);
                    }
